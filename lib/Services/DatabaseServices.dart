...
import 'package:social_media_app/Models/UserModel.dart';
 ...
 class DatabaseServices {
  ...
  static void updateUserData(UserModel user) {
    usersRef.doc(user.id).update({'name': user.name,'bio': user.bio,'profilePicture': user.profilePicture, 'coverImage': user.coverImage,});
  }

  static Future<QuerySnapshot> searchUsers(String name) async {
    Future<QuerySnapshot> users = usersRef.where('name', isGreaterThanOrEqualTo: name).where('name', isLessThan: name + 'z').get();
    return users;
  }
  ...
  static void createPost(Post post) {
    ...
    }).then((doc) async { QuerySnapshot followerSnapshot = await followersRef.doc(post.authorId).collection('Followers').get();
  ...
  static Future<List> getUserPosts(String userId) async {
    QuerySnapshot userPostsSnap = await postsRef.doc(userId).collection('userPosts').orderBy('timestamp', descending: true).get();
...
// QuerySnapshot followersSnapshot = await followersRef.doc(userId).collection('Followers').get();
  ...
  static void unlikePost(String currentUserId, Post post) {
    ...
    DocumentReference postDocFeed = feedRefs.doc(currentUserId).collection('userFeed').doc(post.id);
    postDocFeed.get().then((doc) {
      if (doc.exists) {
        Map<String, dynamic>? data = doc.data() as Map<String, dynamic>?;
        if (data != null) {
...
/** class DatabaseServices {
  ...
  static void createTweet(Post post) {
    tweetsRef.doc(post.authorId).set({'tweetTime': post.timestamp});
    tweetsRef.doc(post.authorId).collection('userTweets').add({
      'text': post.text,
      'image': post.image,
      "authorId": post.authorId,
      "timestamp": post.timestamp,
  ...
  static Future<List> getHomeTweets(String currentUserId) async {
    ...
    homeTweets.docs.map((doc) => Post.fromDoc(doc)).toList();
    return followingTweets;
  }

  static void likeTweet(String currentUserId, Post post) {
    ...
    likesRef.doc(post.id).collection('tweetLikes').doc(currentUserId).set({});
